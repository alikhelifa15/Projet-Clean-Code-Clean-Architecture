...
import {
  ...
  type FormErrors,
} from "../validation/validateFormCompany";
import { z } from "zod";

interface PartialValidationData {
  password?: string;
...
const SignupFormCompany: React.FC = () => {
  ...
  const { registerCompany, isLoading, error } = useRegister();

  const isFormComplete = (data: CompanySignupFormData): boolean => {
    const requiredFields: (keyof CompanySignupFormData)[] = [
      "email",
      "password",
      "confirmPassword",
      "companyName",
  ...
  const validateField = (
    ...
    try {
      ...
      const schemaFields: { [K in keyof PartialValidationData]?: z.ZodType } = {};
      ...
      const partialSchema = z.object(schemaFields);

      let finalSchema = partialSchema;
      if (
        fieldsToValidate.has("password") &&
        fieldsToValidate.has("confirmPassword")
      ) {
        finalSchema = partialSchema.refine(
          (data) => {
            return (
              (!('password' in data) || !('confirmPassword' in data)) ||
      ...
      const dataToValidate = Array.from(fieldsToValidate).reduce(
        (acc, field) => ({
          ...acc,
          [field]: partialData[field as keyof CompanySignupFormData],
        }),
        {} as PartialValidationData
      );
      ...
      if (result.success) {
        ...
        fieldsToValidate.forEach((field) => {
          delete newErrors[field as keyof CompanySignupFormData];
        });
        ...
        result.error.errors.forEach((error) => {
          const field = error.path[0] as keyof CompanySignupFormData;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    validateField(name as keyof CompanySignupFormData, newValue);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validatedData = companySignupSchema.parse(formData);
      ...
      if (error instanceof z.ZodError) {
        ...
        error.errors.forEach((err) => {
          if (err.path[0]) {
            errors[err.path[0] as keyof CompanySignupFormData] = err.message;
          }
        });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {renderInput("companyName", "Nom d'enseigne", "text", "Ex: Ma Société")}
      {renderInput("siretNumber", "Numéro SIRET", "text", "73282932000074")}
